<?php

use Illuminate\Database\Seeder;
use App\model\GradeLesson;


class GradeLessonTableSeeder extends Seeder
{
	/**
	 * Run the database seeds.
	 *
	 * @return void
	 */
	public function run()
	{
		//id 1
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 0;
		$gradeLesson->gradeId       = 1;
		$gradeLesson->lessonId      = 15;
		$gradeLesson->ratio         = 4;
		$gradeLesson->type          = 'GENERAL';
		$gradeLesson->save();


		//id 2
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 0;
		$gradeLesson->gradeId       = 2;
		$gradeLesson->lessonId      = 16;
		$gradeLesson->ratio         = 4;
		$gradeLesson->type          = 'GENERAL';
		$gradeLesson->save();


		//id 3
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 0;
		$gradeLesson->gradeId       = 3;
		$gradeLesson->lessonId      = 17;
		$gradeLesson->ratio         = 4;
		$gradeLesson->type          = 'GENERAL';
		$gradeLesson->save();


		//id 4
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 5;
		$gradeLesson->gradeId       = 1;
		$gradeLesson->lessonId      = 18;
		$gradeLesson->ratio         = 2;
		$gradeLesson->type          = 'GENERAL';
		$gradeLesson->save();


		//id 5
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 5;
		$gradeLesson->gradeId       = 2;
		$gradeLesson->lessonId      = 19;
		$gradeLesson->ratio         = 2;
		$gradeLesson->type          = 'GENERAL';
		$gradeLesson->save();


		//id 6
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 5;
		$gradeLesson->gradeId       = 3;
		$gradeLesson->lessonId      = 20;
		$gradeLesson->ratio         = 2;
		$gradeLesson->type          = 'GENERAL';
		$gradeLesson->save();


		//id 7
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 3;
		$gradeLesson->gradeId       = 1;
		$gradeLesson->lessonId      = 21;
		$gradeLesson->ratio         = 2;
		$gradeLesson->type          = 'GENERAL';
		$gradeLesson->save();


		//id 8
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 3;
		$gradeLesson->gradeId       = 2;
		$gradeLesson->lessonId      = 22;
		$gradeLesson->ratio         = 2;
		$gradeLesson->type          = 'GENERAL';
		$gradeLesson->save();


		//id 9
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 3;
		$gradeLesson->gradeId       = 3;
		$gradeLesson->lessonId      = 23;
		$gradeLesson->ratio         = 2;
		$gradeLesson->type          = 'GENERAL';
		$gradeLesson->save();


		//id 10
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 5;
		$gradeLesson->gradeId       = 1;
		$gradeLesson->lessonId      = 24;
		$gradeLesson->ratio         = 3;
		$gradeLesson->type          = 'GENERAL';
		$gradeLesson->save();


		//id 11
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 5;
		$gradeLesson->gradeId       = 2;
		$gradeLesson->lessonId      = 25;
		$gradeLesson->ratio         = 3;
		$gradeLesson->type          = 'GENERAL';
		$gradeLesson->save();


		//id 12
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 5;
		$gradeLesson->gradeId       = 3;
		$gradeLesson->lessonId      = 26;
		$gradeLesson->ratio         = 3;
		$gradeLesson->type          = 'GENERAL';
		$gradeLesson->save();


		//id 13
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 3;
		$gradeLesson->gradeId       = 1;
		$gradeLesson->lessonId      = 27;
		$gradeLesson->ratio         = 3;
		$gradeLesson->type          = 'GENERAL';
		$gradeLesson->save();


		//id 14
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 3;
		$gradeLesson->gradeId       = 2;
		$gradeLesson->lessonId      = 28;
		$gradeLesson->ratio         = 3;
		$gradeLesson->type          = 'GENERAL';
		$gradeLesson->save();


		//id 15
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 3;
		$gradeLesson->gradeId       = 3;
		$gradeLesson->lessonId      = 29;
		$gradeLesson->ratio         = 3;
		$gradeLesson->type          = 'GENERAL';
		$gradeLesson->save();


		//id 16
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 0;
		$gradeLesson->gradeId       = 1;
		$gradeLesson->lessonId      = 30;
		$gradeLesson->ratio         = 2;
		$gradeLesson->type          = 'GENERAL';
		$gradeLesson->save();


		//id 17
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 0;
		$gradeLesson->gradeId       = 2;
		$gradeLesson->lessonId      = 31;
		$gradeLesson->ratio         = 2;
		$gradeLesson->type          = 'GENERAL';
		$gradeLesson->save();


		//id 18
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 0;
		$gradeLesson->gradeId       = 3;
		$gradeLesson->lessonId      = 32;
		$gradeLesson->ratio         = 2;
		$gradeLesson->type          = 'GENERAL';
		$gradeLesson->save();

		/*Experts*/


		//id 19
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 2;
		$gradeLesson->gradeId       = 1;
		$gradeLesson->lessonId      = 33;
		$gradeLesson->ratio         = 4;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 20
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 2;
		$gradeLesson->gradeId       = 1;
		$gradeLesson->lessonId      = 34;
		$gradeLesson->ratio         = 4;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 21
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 2;
		$gradeLesson->gradeId       = 2;
		$gradeLesson->lessonId      = 35;
		$gradeLesson->ratio         = 4;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 22
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 2;
		$gradeLesson->gradeId       = 2;
		$gradeLesson->lessonId      = 36;
		$gradeLesson->ratio         = 4;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 23
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 2;
		$gradeLesson->gradeId       = 2;
		$gradeLesson->lessonId      = 37;
		$gradeLesson->ratio         = 4;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 24
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 2;
		$gradeLesson->gradeId       = 3;
		$gradeLesson->lessonId      = 38;
		$gradeLesson->ratio         = 4;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 25
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 2;
		$gradeLesson->gradeId       = 3;
		$gradeLesson->lessonId      = 39;
		$gradeLesson->ratio         = 4;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 26
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 2;
		$gradeLesson->gradeId       = 3;
		$gradeLesson->lessonId      = 40;
		$gradeLesson->ratio         = 4;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 27
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 2;
		$gradeLesson->gradeId       = 1;
		$gradeLesson->lessonId      = 41;
		$gradeLesson->ratio         = 3;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 28
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 2;
		$gradeLesson->gradeId       = 2;
		$gradeLesson->lessonId      = 42;
		$gradeLesson->ratio         = 3;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 29
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 2;
		$gradeLesson->gradeId       = 3;
		$gradeLesson->lessonId      = 43;
		$gradeLesson->ratio         = 3;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 30
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 2;
		$gradeLesson->gradeId       = 1;
		$gradeLesson->lessonId      = 44;
		$gradeLesson->ratio         = 3;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 31
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 2;
		$gradeLesson->gradeId       = 2;
		$gradeLesson->lessonId      = 45;
		$gradeLesson->ratio         = 3;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 32
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 2;
		$gradeLesson->gradeId       = 3;
		$gradeLesson->lessonId      = 46;
		$gradeLesson->ratio         = 3;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 33
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 3;
		$gradeLesson->gradeId       = 2;
		$gradeLesson->lessonId      = 47;
		$gradeLesson->ratio         = 1;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 34
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 3;
		$gradeLesson->gradeId       = 1;
		$gradeLesson->lessonId      = 33;
		$gradeLesson->ratio         = 3;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 35
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 3;
		$gradeLesson->gradeId       = 2;
		$gradeLesson->lessonId      = 48;
		$gradeLesson->ratio         = 3;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 36
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 3;
		$gradeLesson->gradeId       = 3;
		$gradeLesson->lessonId      = 49;
		$gradeLesson->ratio         = 3;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 37
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 3;
		$gradeLesson->gradeId       = 1;
		$gradeLesson->lessonId      = 50;
		$gradeLesson->ratio         = 4;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 38
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 3;
		$gradeLesson->gradeId       = 2;
		$gradeLesson->lessonId      = 51;
		$gradeLesson->ratio         = 4;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 39
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 3;
		$gradeLesson->gradeId       = 3;
		$gradeLesson->lessonId      = 52;
		$gradeLesson->ratio         = 4;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 40
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 3;
		$gradeLesson->gradeId       = 1;
		$gradeLesson->lessonId      = 41;
		$gradeLesson->ratio         = 2;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 41
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 3;
		$gradeLesson->gradeId       = 2;
		$gradeLesson->lessonId      = 42;
		$gradeLesson->ratio         = 2;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 42
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 3;
		$gradeLesson->gradeId       = 3;
		$gradeLesson->lessonId      = 43;
		$gradeLesson->ratio         = 2;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 43
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 3;
		$gradeLesson->gradeId       = 1;
		$gradeLesson->lessonId      = 44;
		$gradeLesson->ratio         = 3;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 44
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 3;
		$gradeLesson->gradeId       = 2;
		$gradeLesson->lessonId      = 45;
		$gradeLesson->ratio         = 3;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 45
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 3;
		$gradeLesson->gradeId       = 3;
		$gradeLesson->lessonId      = 46;
		$gradeLesson->ratio         = 3;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 46
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 1;
		$gradeLesson->gradeId       = 1;
		$gradeLesson->lessonId      = 53;
		$gradeLesson->ratio         = 3;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 47
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 1;
		$gradeLesson->gradeId       = 2;
		$gradeLesson->lessonId      = 54;
		$gradeLesson->ratio         = 3;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 48
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 1;
		$gradeLesson->gradeId       = 3;
		$gradeLesson->lessonId      = 55;
		$gradeLesson->ratio         = 3;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 49
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 1;
		$gradeLesson->gradeId       = 1;
		$gradeLesson->lessonId      = 56;
		$gradeLesson->ratio         = 2;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 50
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 1;
		$gradeLesson->gradeId       = 1;
		$gradeLesson->lessonId      = 57;
		$gradeLesson->ratio         = 2;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 51
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 1;
		$gradeLesson->gradeId       = 2;
		$gradeLesson->lessonId      = 58;
		$gradeLesson->ratio         = 2;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 52
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 1;
		$gradeLesson->gradeId       = 3;
		$gradeLesson->lessonId      = 59;
		$gradeLesson->ratio         = 2;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 53
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 1;
		$gradeLesson->gradeId       = 1;
		$gradeLesson->lessonId      = 60;
		$gradeLesson->ratio         = 1;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 54
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 1;
		$gradeLesson->gradeId       = 2;
		$gradeLesson->lessonId      = 61;
		$gradeLesson->ratio         = 1;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 55
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 1;
		$gradeLesson->gradeId       = 3;
		$gradeLesson->lessonId      = 62;
		$gradeLesson->ratio         = 1;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 56
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 1;
		$gradeLesson->gradeId       = 1;
		$gradeLesson->lessonId      = 63;
		$gradeLesson->ratio         = 1;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 57
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 1;
		$gradeLesson->gradeId       = 2;
		$gradeLesson->lessonId      = 64;
		$gradeLesson->ratio         = 1;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 58
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 1;
		$gradeLesson->gradeId       = 3;
		$gradeLesson->lessonId      = 65;
		$gradeLesson->ratio         = 1;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 59
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 1;
		$gradeLesson->gradeId       = 1;
		$gradeLesson->lessonId      = 66;
		$gradeLesson->ratio         = 2;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 60
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 1;
		$gradeLesson->gradeId       = 2;
		$gradeLesson->lessonId      = 67;
		$gradeLesson->ratio         = 2;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 61
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 1;
		$gradeLesson->gradeId       = 3;
		$gradeLesson->lessonId      = 68;
		$gradeLesson->ratio         = 2;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 62
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 1;
		$gradeLesson->gradeId       = 2;
		$gradeLesson->lessonId      = 69;
		$gradeLesson->ratio         = 2;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 63
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 1;
		$gradeLesson->gradeId       = 3;
		$gradeLesson->lessonId      = 70;
		$gradeLesson->ratio         = 2;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 64
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 1;
		$gradeLesson->gradeId       = 1;
		$gradeLesson->lessonId      = 71;
		$gradeLesson->ratio         = 2;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();


		//id 65
		$gradeLesson                = new GradeLesson();
		$gradeLesson->orientationId = 1;
		$gradeLesson->gradeId       = 2;
		$gradeLesson->lessonId      = 72;
		$gradeLesson->ratio         = 3;
		$gradeLesson->type          = 'Expert';
		$gradeLesson->save();



	}
}
